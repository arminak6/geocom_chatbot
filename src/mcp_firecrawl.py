...
from utils import canonical_site_host, extract_json_from_model_output, normalize_session_url, site_url_variants
...
async def firecrawl_map_subpages_with_metadata_via_mcp(
    session: ClientSession,
    ...
    for seed_url in site_url_variants(base_url):
        ...
        if getattr(result, "isError", False):
            ...
            last_error = RuntimeError(f"Firecrawl MCP map error for {seed_url}: {msg}")
            continue
        ...
        def add_candidate(candidate: str):
            nonlocal variant_candidates, seen, base_domain, parsed_seed, base_session_url
            if not candidate:
                return
            candidate = candidate.strip()
            if candidate.startswith("//"):
                candidate = parsed_seed.scheme + ":" + candidate
            if candidate.startswith("/"):
                candidate = f"{parsed_seed.scheme}://{parsed_seed.netloc}{candidate}"

            parsed = urlparse(candidate)
            if not parsed.scheme.startswith("http"):
                return
            if canonical_site_host(candidate) != base_domain:
                return
            if normalize_session_url(candidate) == base_session_url:
                return
        ...
        if variant_candidates:
            ...
            if seed_url != base_url:
                print(f"[map] Retried subpage discovery with alternate host: {seed_url}")
            break

    if not candidates:
        if last_error is not None:
            print(f"[warning] {last_error}")
        print("[info] Firecrawl 'map' returned no same-domain links.")
        return {
            "candidate_urls": [],
            "selected_urls": [],
            "url_scores": {},
    ...
    print(f"\n[map] Map API found {len(candidates)} candidate URLs:")
    ...
    print("[map] LLM-based URL scores:", url_scores)
    scored: List[tuple] = []
    ...
    print("[map] Deep dive: ranked subpage candidates (score, url):")
    for s, idx, u in top:
...
async def firecrawl_map_subpages_via_mcp(
    ...
    ranking = await firecrawl_map_subpages_with_metadata_via_mcp(
        session=session,
    ...
    return list(ranking.get("selected_urls", []))
